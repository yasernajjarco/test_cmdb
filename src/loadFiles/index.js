...
/**
 * open all files in folders (Documents and uodate company)
 * and analyse it one by one to insert in DB
 */
async function initialLoad() {
    ...
    await insertVirtuels.insert(file, file.substring(file.lastIndexOf(" ")).trim().charAt(0)).then(async(data) => {
        ...
        if (data !== undefined) {
            ...
            await XLSX.writeFile(wb, path.resolve(__dirname, 'Exported_files_forCompany/', 'linux_clients Z.xlsx'));
        }
    });
    ...
    await instances.insertInstances(file, file.substring(file.lastIndexOf(" ")).trim().charAt(0)).then(async(data) => {
        ...
        // await XLSX.writeFile(wb, path.resolve(__dirname, 'Exported_files_forCompany/', 'sinstances_clients B.xlsx'));
    ...
    await insertOccurence.insert(file, file.substring(file.lastIndexOf(" ")).trim().charAt(0)).then(async(data) => {
        ...
        //  await XLSX.writeFile(wb, path.resolve(__dirname, 'Exported_files_forCompany/', 'occurences_clients B.xlsx'));
    });
    ...
    await insertOccurence.insert(file, file.substring(file.lastIndexOf(" ")).trim().charAt(0)).then(async(data) => {
        ...
        //  await XLSX.writeFile(wb, path.resolve(__dirname, 'Exported_files_forCompany/', 'occurences_clients Z.xlsx'));
    });
...
async function generateFiles() {
    ...
    await removeElementsNoOccurenciables(clients_instance_z).then(result => {
        ...
        XLSX.writeFile(wb, path.resolve(__dirname, 'Exported_files_forCompany/', 'sinstances_clients Z.xlsx'));

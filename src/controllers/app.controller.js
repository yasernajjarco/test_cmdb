...
//,     [Sequelize.literal('"ci"."company"'), 'companyName']
 app.findAll({
  ...
  include: [
    ...
    { 
       model: db.instance, required: false, as: 'instance',  attributes: ['name','isoccurenciable'], 
    },
    { 
      model: db.provider, required: false, as: 'provider',  attributes: [], 
    }

],
attributes: ['version', 'is_valid',[Sequelize.col('ci.platforms.name'), 'platform'],[Sequelize.col('provider.name'), 'providerName'],[Sequelize.col('ci.company'), 'company'] , [Sequelize.col('ci.description'), 'description']]
...
app.findAll({
  include: [
    ...
    { 
      model: db.provider, required: true, as: 'provider',  where : { provider_id : id },
    }

],
attributes: ['version','product_code', 'is_valid',[Sequelize.col('provider.name'), 'providerName'],[Sequelize.col('ci.company'), 'company'] , [Sequelize.col('ci.description'), 'description']]
...
app.findAll({
  include: [
    {  model: db.ci,  required: true, as: 'ci', attributes: [],
    ...
    { 
      model: db.provider, required: true, as: 'provider',  attributes: [],
    }
